...
public:
	...
	void Insert(FPrimitiveProxy* Proxy, const FBoundingBox& Bounds);
	//	프러스텀과 겹치는 후보 프록시만 수집
	void Query(const FFrustumPlanes& Frustum, TArray<FPrimitiveProxy*>& OutProxies) const;
	const FOctreeDebugStats& GetLastDebugStats() const { return LastDebugStats; }
...
private:
	...
	void QueryNode(const FNode& Node, const FFrustumPlanes& Frustum, TArray<FPrimitiveProxy*>& OutProxies) const;
	...
	mutable FOctreeDebugStats LastDebugStats;
};
