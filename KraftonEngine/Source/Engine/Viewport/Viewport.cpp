...
bool FViewport::CreateResources()
{
	...
	RTTexture->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportSceneColorTexture"), "ViewportSceneColorTexture");

	hr = Device->CreateRenderTargetView(RTTexture, nullptr, &RTV);
	if (FAILED(hr)) return false;
	RTV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportSceneColorRTV"), "ViewportSceneColorRTV");

	hr = Device->CreateShaderResourceView(RTTexture, nullptr, &SRV);
	if (FAILED(hr)) return false;
	SRV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportSceneColorSRV"), "ViewportSceneColorSRV");
	...
	if (FAILED(hr)) return false;
	SceneColorCopyTexture->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportSceneColorCopyTexture"), "ViewportSceneColorCopyTexture");
	...
	if (FAILED(hr)) return false;
	DepthTexture->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportDepthTexture"), "ViewportDepthTexture");
	...
	if (FAILED(hr)) return false;
	DSV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportDSV"), "ViewportDSV");
	...
	if (FAILED(hr)) return false;
	DepthCopyTexture->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportDepthCopyTexture"), "ViewportDepthCopyTexture");

	hr = Device->CreateShaderResourceView(DepthCopyTexture, &DepthSRVDesc, &DepthCopySRV);
	if (FAILED(hr)) return false;
	DepthCopySRV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportDepthCopySRV"), "ViewportDepthCopySRV");

	hr = Device->CreateShaderResourceView(DepthCopyTexture, &StencilSRVDesc, &StencilCopySRV);
	if (FAILED(hr)) return false;
	StencilCopySRV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportStencilCopySRV"), "ViewportStencilCopySRV");
	...
	if (FAILED(hr)) return false;
	SceneColorCopySRV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportSceneColorCopySRV"), "ViewportSceneColorCopySRV");
	...
	if (FAILED(hr)) return false;
	NormalTexture->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportNormalTexture"), "ViewportNormalTexture");

	hr = Device->CreateRenderTargetView(NormalTexture, nullptr, &NormalRTV);
	if (FAILED(hr)) return false;
	NormalRTV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportNormalRTV"), "ViewportNormalRTV");

	hr = Device->CreateShaderResourceView(NormalTexture, nullptr, &NormalSRV);
	if (FAILED(hr)) return false;
	NormalSRV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportNormalSRV"), "ViewportNormalSRV");
	...
	if (FAILED(hr)) return false;
	CullingHeatmapTexture->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportCullingHeatmapTexture"), "ViewportCullingHeatmapTexture");

	hr = Device->CreateRenderTargetView(CullingHeatmapTexture, nullptr, &CullingHeatmapRTV);
	if (FAILED(hr)) return false;
	CullingHeatmapRTV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportCullingHeatmapRTV"), "ViewportCullingHeatmapRTV");

	hr = Device->CreateShaderResourceView(CullingHeatmapTexture, nullptr, &CullingHeatmapSRV);
	if (FAILED(hr)) return false;
	CullingHeatmapSRV->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)strlen("ViewportCullingHeatmapSRV"), "ViewportCullingHeatmapSRV");
