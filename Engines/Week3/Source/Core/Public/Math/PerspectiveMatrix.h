...
template <typename T> struct FPerspectiveMatrix : public FMatrix<T>
{
...
FPerspectiveMatrix<T>::FPerspectiveMatrix(T HalfFOVX, T HalfFOVY, T MultFOVX, T MultFOVY, T MinZ, T MaxZ)
    : FMatrix<T>(FPlane<T>(MultFOVX / tan(HalfFOVX), 0.0f, 0.0f, 0.0f),
                 FPlane<T>(0.0f, MultFOVY / tan(HalfFOVY), 0.0f, 0.0f),
...
template <typename T> FMatrix<T> FPerspectiveMatrix<T>::Inverse() const
{
